...
Status VideoGradeImpl::Init(std::shared_ptr<TNNSDKOption> option_i) {
  Status status = TNN_OK;
  auto option = dynamic_cast<VideoGradeOption *>(option_i.get());
...
std::shared_ptr<Mat> VideoGradeImpl::ProcessSDKInputMat(std::shared_ptr<Mat> input_mat,
                                                           std::string name) {
  return TNNSDKSample::ResizeToInputShape(input_mat, name);
}

MatConvertParam VideoGradeImpl::GetConvertParamForInput(std::string tag) {
...
std::shared_ptr<TNNSDKOutput> VideoGradeImpl::CreateSDKOutput() {
  return std::make_shared<VideoGradeOutput>();
}

Status VideoGradeImpl::ProcessSDKOutput(std::shared_ptr<TNNSDKOutput> output_) {
  Status status = TNN_OK;
  auto option = dynamic_cast<VideoGradeOption *>(option_.get());
  RETURN_VALUE_ON_NEQ(!option, false,
                      Status(TNNERR_PARAM_ERR, "TNNSDKOption is invalid"));
  auto output = dynamic_cast<VideoGradeOutput *>(output_.get());
...
void VideoGradeImpl::GenerateBBox(std::vector<BlazeFaceInfo> &detects, TNN_NS::Mat &scores, TNN_NS::Mat &boxes, int image_w, int image_h, float min_score_threshold) {
...
void VideoGradeImpl::BlendingNMS(std::vector<BlazeFaceInfo> &input, std::vector<BlazeFaceInfo> &output, float min_suppression_threshold) {
